...
impl Tactictoe{
    ...
    pub fn game_start_cli(&mut self){
        ...
        self.show_map("");
        loop{
            let ans = input(&format!("\x1B[{}Hplease enter the the coordinate (1-{})\n>",self.map.map.len()*2+3,coordinate.len()));
            ...
            key = match ans.trim().to_lowercase().as_str().parse(){
                Ok(a)=>a,
                Err(_)=>{
                        self.show_map(&format!("
\x1B[2;{0}H❗Oops, seems like your input isnt a number. Please try again❗
\x1B[3;{0}H💡If you want to quit, please enter %q💡
\x1B[4;{0}H💡If you want to restart the game, please enter %r💡"
                        ,err_text_col));
                    continue;
                    }
            };
            if !(1..=coordinate.len()).contains(&key){
                self.show_map(&format!("\x1B[2;{}H❗Oops, looks like the input isn't in range (1–{}). Please try again❗",err_text_col,coordinate.len()));
                continue;
            }
            ...
            if !self.put_player_symbol( &symbol, coor[0], coor[1]){
                self.show_map(&format!("\x1B[2;{}H❗Oops, you can't cover the enemy symbol. Please try again❗",err_text_col));
                continue;
            }
